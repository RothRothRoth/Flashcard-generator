...
export default function AccountPage() {
  ...
  const [username, setUsername] = useState<string>("Roth");
    ...
    <main className="min-h-screen flex bg-[#F6F7FB]">
      ...
      <section className="flex-1 px-4 sm:px-6 md:px-8 lg:px-10 py-6">
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => setCollapsed(!collapsed)} className="bg-white shadow rounded-lg p-2">
            <Image src="/menu.png" alt="menu" width={18} height={18} />
          </button>
          <button onClick={() => router.push("/account")} className="w-12 h-12 rounded-full bg-[#646DE8] shadow-md flex items-center justify-center">
        ...
        <div className="max-w-4xl mx-auto w-full">
          <h1 className="text-3xl font-bold text-[#646DE8] text-center mb-8">Account Settings</h1>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
            <div className="flex flex-col md:flex-row items-start justify-between gap-8">
              ...
              <div className="flex-1 space-y-6 w-full">
                <div>
                  <label className="block text-sm font-medium text-[#646DE8] mb-1">Your Name</label>
                  <div className="relative">
                    <input
                      value={username}
                      disabled={!editing || loading}
                      onChange={(e) => setUsername(e.target.value)}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#646DE8]"
                    />
                    {!editing && (
                      <button onClick={() => setEditing(true)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#646DE8] mb-1">Email Address</label>
                  <input value={email} disabled className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-[#646DE8] mb-1">Password</label>
                  <input value="********" disabled className="w-full px-4 py-2.5 border border-gray-300 rounded-lg bg-gray-50" />
                </div>

                {editing && (
                  <div className="flex gap-4 pt-2">
                    <button onClick={handleSave} disabled={loading} className="bg-[#646DE8] text-white px-6 py-2.5 rounded-lg text-sm hover:bg-[#5a63d0] transition flex-1 disabled:opacity-50">
                      {loading ? "Saving..." : "Save"}
                    </button>
                    <button onClick={() => setEditing(false)} className="bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg text-sm hover:bg-gray-300 transition flex-1">
                      Cancel
                    </button>
                  </div>
                )}
...
function Sidebar({ collapsed, onLogout }: { collapsed: boolean; onLogout: () => void }) {
  const router = useRouter();
  return (
    <aside className={`${collapsed ? "w-20" : "w-64"} bg-[#646DE8] text-white flex flex-col items-center py-12 px-6 transition-all duration-300`}>
      ...
      <button onClick={onLogout} className="mt-auto bg-white text-[#646DE8] rounded-2xl py-3 w-full flex items-center justify-center gap-3 text-sm font-semibold">
